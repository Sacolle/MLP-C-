...
void Funcionario::set_dt_ingresso(time_t dt){
    if(dt < dt_nascimento ){
        throw std::runtime_error("data de ingresso menor que data de nascimento " + std::to_string(dt) + " < " + std::to_string(dt_nascimento));
    }
    const auto t = time(0);
    if(dt > t){
        throw std::runtime_error("data de ingresso  menor que o momento atual " + std::to_string(dt) + " > " + std::to_string(t));
    }
    this->dt_ingresso = dt;
}
